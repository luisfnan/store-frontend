...
function Suppliers() {
    ...
    async function getData() {
        const response = await axios.get('https://store-api-l9ki.onrender.com/api/suppliers')
    ...
    if (supplier[0]) {
        objectKeys = Object.keys(supplier[0]).map((key) => {
            return {
                Header: key,
                accessor: key,
            }
    ...
    const categoryData = useMemo(() => [...supplier], [supplier])
    const categoryColumn = useMemo(() => objectKeys, [supplier])
