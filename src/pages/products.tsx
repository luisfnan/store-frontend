...
function Suppliers() {
    ...
    async function getData() {
        const response = await axios.get('https://store-api-l9ki.onrender.com/api/products')
    ...
    if (product[0]) {
        ...
        objectKeys = Object.keys(product[0]).filter((key) => key !== "categoryId").filter((key) => key !== "supplierID").filter((key) => key !== "category").map((key) => {
            return {
                Header: key,
                accessor: key,
            }
    ...
    const categoryData = useMemo(() => [...product], [product])
    const categoryColumn = useMemo(() => objectKeys, [product])
